...
class ModelRouter:
    ...
    def _detect_local_models(self) -> Dict[str, ModelConfig]:
        ...
        try:
            ...
            print(f"[ModelRouter] Detected {len(detected)} local models: {list(detected.keys())}")
            return detected
    ...
    def _init_routing_rules(self) -> Dict[TaskType, Dict]:
        ...
        return {
            TaskType.LOCATION_DESCRIPTION: {
                ...
                "tokens": 250,
                "temperature": 0.75,
                "priority_boost": {"gemma": 1, "mistral": 0}  # Prefer creative for locations
            },
            TaskType.QUICK_CHOICE: {
                ...
                "tokens": 100,
                "temperature": 0.7,
                "priority_boost": {"llama3.2": 2, "phi": 2}  # Strongly prefer fast models
            },
            TaskType.DIALOGUE: {
    ...
    def select_model(self, prompt: str, context: str = "", task_type: Optional[TaskType] = None) -> Tuple[str, Dict]:
        """
        ...
        self.stats["by_model"][best_model] = self.stats["by_model"].get(best_model, 0) + 1
        self.stats["by_task"][task_type.value] = self.stats["by_task"].get(task_type.value, 0) + 1

        print(f"[ModelRouter] Task: {task_type.value}, Selected: {best_model}, Options: {options}")
